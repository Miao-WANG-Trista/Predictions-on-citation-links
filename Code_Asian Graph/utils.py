...
class GraphStructure:
    ...
    def __init__(self, c, load_from_file: bool):
        ...
        if load_from_file:
            for d in self.handled_dicts:
                try:
                    self.graph_dicts[d] = np.load("%sdict_%s.npy" % (self.save_dir, d), allow_pickle=True).item()
                    print("Loaded %s dictionary!" % d)
                except FileNotFoundError:
                    print("Could not load %s." % d)
            try:
                self.partition = np.load("%spartition.npy" % self.save_dir, allow_pickle=True).item()
                print("Loaded partition file.")
                for node in self.g.nodes():
                  self.g.nodes[node]['community'] = self.partition[node]
                print('Add Community Attributes')
